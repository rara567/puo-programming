...
st.markdown(f"""
    ...
    .login-header {{ text-align: center; color: white; font-size: 3rem; font-weight: bold; margin-bottom: 2rem; font-family: 'Segoe UI', sans-serif; }}
    ...
    #video-bg {{ position: absolute; top: 50%; left: 50%; min-width: 100%; min-height: 100%; transform: translate(-50%, -50%); object-fit: cover; }}
...
if not st.session_state.logged_in:
    ...
    if uploaded_file is not None:
        try:
            ...
            if sat_toggle:
                # MOD SATELIT
                m = folium.Map(location=[df_mapped['lat'].mean(), df_mapped['lon'].mean()], zoom_start=20, max_zoom=22)
                t_type = 'y' if map_selection == "Satalit (Hybrid)" else 'm'
                folium.TileLayer(tiles=f'https://mt1.google.com/vt/lyrs={t_type}&x={{x}}&y={{y}}&z={{z}}', attr='Google', max_zoom=22, max_native_zoom=19).add_to(m)
                folium.Polygon([[la, lo] for lo, la in coords_wgs_closed], color="yellow", weight=3, fill=True, fill_opacity=0.3).add_to(m)
                if show_area_label:
                    folium.Marker([df_mapped['lat'].mean(), df_mapped['lon'].mean()], icon=folium.DivIcon(html=f'<div style="color:#2ecc71; font-weight:bold; font-size:{area_font_size}pt; background:white; padding:2px 5px; border:1px solid #2ecc71; border-radius:3px;">{fixed_area:.2f} m²</div>')).add_to(m)
                ...
                # MOD GRAF (PENAMBAHBAIKAN DI SINI)
                st.markdown("### 📊 Paparan Lot & Graf Teknikal")
                fig, ax = plt.subplots(figsize=(12, 8))
                
                # Sediakan Polygon & Plot
                pts = np.array(coords_local_closed)
                ax.plot(pts[:,0], pts[:,1], color='yellow', linewidth=2.5, zorder=3)
                ax.add_patch(MatplotlibPolygon(pts, facecolor='#D8BFD8', alpha=0.6, zorder=2))

                # Format Saintifik Paksi-X (Sama seperti gambar rujukan)
                ax.xaxis.set_major_formatter(ScalarFormatter(useMathText=True))
                ax.ticklabel_format(style='sci', axis='x', scilimits=(0,0))
                ax.grid(True, linestyle='--', alpha=0.5)

                # Label Bearing & Jarak (Teks Merah)
                for i, ml in enumerate(mid_l):
                    ...
                    ax.text(ml[0], ml[1], f"{bearings[i].replace(chr(34), chr(34)+chr(10))}{distances[i]:.2f}m", 
                            color='brown', fontsize=bearing_font_size+2, fontweight='bold', ha='center', va='center', rotation=rot)

                # Label Luas (Kotak Putih Border Hijau)
                if show_area_label:
                    c = poly_local.centroid
                    ax.text(c.x, c.y, f"{fixed_area:.2f} m²", color='green', fontsize=area_font_size+2, 
                            fontweight='bold', ha='center', bbox=dict(facecolor='white', edgecolor='green', boxstyle='round,pad=0.3'))
                
                # Stesen (Bulatan Merah No Putih)
                for i, r in df_mapped.iterrows():
                    ax.scatter(r['E'], r['N'], color='red', edgecolor='black', s=station_circle_size*5, zorder=5)
                    ax.annotate(str(int(r['STN'])), (r['E'], r['N']), xytext=(station_label_offset*8, station_label_offset*8), 
                                textcoords='offset points', color='black', fontweight='bold', fontsize=11,
                                bbox=dict(facecolor='white', edgecolor='red', boxstyle='circle', alpha=0.8))
